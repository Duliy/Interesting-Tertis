...
#define RECORDED_FILE "record.txr"
...
// 信息区 - 下一个文字坐标
CONST RectPosition NEXT_FONT_POSITION(BLOCK_SIZE*2 + NODE_SIZE * GAME_XNODE + BLOCK_SIZE, BLOCK_SIZE * 2 + 2 * NODE_SIZE,PAINT_WIDTH - BLOCK_SIZE * 2, BLOCK_SIZE * 2 + 2 * NODE_SIZE);
// 信息区 - 作者坐标
CONST RectPosition Writer_POSITION(BLOCK_SIZE * 2 + NODE_SIZE * GAME_XNODE + BLOCK_SIZE, BLOCK_SIZE * 2 , PAINT_WIDTH - BLOCK_SIZE * 2 , BLOCK_SIZE * 2 + 2 * NODE_SIZE );
// 信息区 - 制作组坐标
CONST RectPosition TEAM_POSITION(BLOCK_SIZE * 2 + NODE_SIZE * GAME_XNODE + BLOCK_SIZE, BLOCK_SIZE * 2 + 1 * NODE_SIZE, PAINT_WIDTH - BLOCK_SIZE * 2, BLOCK_SIZE * 2 + 2 * NODE_SIZE);
// 信息区 - 下一个图形坐标
...
// 信息区 - 炸弹数量
CONST RectPosition BOOM_FONT_POSITION(NODE_SIZE* GAME_XNODE + NODE_SIZE * 2, BLOCK_SIZE * 2 + 12 * NODE_SIZE, PAINT_WIDTH - BLOCK_SIZE - NODE_SIZE, BLOCK_SIZE * 2 + 13 * NODE_SIZE);
// 信息区 - 时停怀表数量
CONST RectPosition CLOCK_FONT_POSITION(NODE_SIZE* GAME_XNODE + NODE_SIZE * 2, BLOCK_SIZE * 2 + 13 * NODE_SIZE, PAINT_WIDTH - BLOCK_SIZE - NODE_SIZE, BLOCK_SIZE * 2 + 14 * NODE_SIZE);
// 信息区 - 最高分
CONST RectPosition HIGHEST_FONT_POSITION(NODE_SIZE* GAME_XNODE + NODE_SIZE * 2, BLOCK_SIZE * 2 + 14 * NODE_SIZE, PAINT_WIDTH - BLOCK_SIZE - NODE_SIZE, BLOCK_SIZE * 2 + 15 * NODE_SIZE);
...
UINT iCurBoom = 0;	   // 当前炸弹数量
UINT iCurClock = 0;    // 当前时停怀表数量
UINT totalBoom = 0;	   // 总共炸弹数量
UINT totalClock = 0;    // 总共时停怀表数量
UINT Highestsocre = 0;
UINT resurrection = 0;
/* 游戏区域栅栏 ,值表示对应颜色枚举类型*/
